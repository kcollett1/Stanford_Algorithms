...
'''
    Using my quicksort implementation from previous course to sort edges of an
    undirected weighted graph to apply to kruskal's MST finding greedy algorithm.
    This has been slightly modified to take as input the edges dict from my graph
    data structure, sort them by increasing order of edge cost, and return an
    iterator of corresponding edge numbers to iterate over.
'''
    ...
    for scan in range(left_stop, stop): # O(n)
        if arr[scan][0] < arr[start][0]:
            ...
            if left_stop != scan or left_stop != dup_stop:  # we've seen a smaller/larger val
                ...
                if left_stop != scan and left_stop != dup_stop:  # we've seen BOTH smaller/larger val
                    arr[scan],arr[left_stop] = arr[left_stop],arr[scan]
            # increment duplicate pntr and end of smaller array every time we encounter this situation
            dup_stop += 1
            left_stop += 1
...
def quick_sort(arr, piv_choice='rand'):
    '''
    ...
    '''
    # pre-process arr, we will sort arr by first element, while remembering correct
    # ordering of second element. in the specific application of this to kruskal's algo,
    # we are sorting by edge cost, and remembering the corresponding edge# order
    arr_sort = [[val[2], e] for e,val in enumerate(arr)]  # O(n)
    quick_sort_recurse(arr_sort, 0, len(arr_sort), piv_choice)  # O(nlog(n))
    return [arr[v[1]] for v in arr_sort]


def quick_sort_recurse(arr: list, start=None, stop=None, piv_choice='rand'):
    '''
    ...
    '''
    if start == None:  # default case, sort from beginning of array if start not specified
        start = 0
    if stop == None:  # default case, sort until end of array if stop not specified
        stop = len(arr)
