...
'''
    Using my quicksort implementation from previous course to sort weights of alphabet
    characters, modified slightly to maintain ordering of original list indices.
'''
...
def quick_sort(arr: list, start=None, stop=None, piv_choice='rand'):
    '''
    ...
    '''
    if start == None:  # default start sorting arr from beginning of list if no value specified
        start = 0
    if stop == None:  # default end sorting array at end of list if no value specified
        stop = len(arr)
