...
class StackNode:  # hold whatever type of data you're using in your stack
    def __init__(self, val=None, prv=None):
        self.val = val  # of type whatever data you're interested in using
        self.prv = prv  # of type StackNode


class Stack:
    def __init__(self, val=None, status=None):
        if val != None:  # initializing stack with a value
            self.size = 1
            self.head = StackNode(val=val, prv=None)
    ...
    def push(self, val):
        ...
        self.size += 1
        self.head = StackNode(val=val, prv=self.head)
        return val
