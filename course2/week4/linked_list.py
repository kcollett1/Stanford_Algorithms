...
'''
   ...
   adapted for use in my hash dict, storing both a key and a val.
'''


class Node:
    def __init__(self, key=None, val=None, nex=None, prv=None):
        self.key = key
        self.val = val  # this is of whatever type the data you're interested in is
        self.nex = nex
        self.prv = prv
...
class LinkedList:
    ...
    def push_front(self, key, val):
        ...
        self.head = Node(key=key, val=val, nex=self.head, prv=None)
        ...
        return val  # return val to indicate success
    ...
    def pop_front(self):
        ...
        return top_val  # return val removed to indicate success


    def push_back(self, key, val):
        '''Add val to end of list, return val to indicate success'''
        self.l += 1  # increment length of list
        self.tail = Node(key=key, val=val, nex=None, prv=self.tail)
    ...
    def remove_val(self, val):
        '''
           Remove node with val=val from list. Removes first instance of val
    ...
    def remove_key(self, key):
        '''
           Remove node with key=key from list. Removes first instance of key
           Return val corresponding to key to indicate success, or None if
           val is not in list. O(n)
        '''
        traverse = self.head
        while traverse:
            if traverse.key == key: # found node to remove, remove it, and return val
                return self.__remove__(traverse)  # update pointers and return val

            traverse = traverse.nex

        return None  # made it through whole list and did not find key
    ...
    def insert_ith(self, i, key, val):
        '''
           Insert a node with key/val pair into the i-th (1-based) position in the list.
        ...
        if i == self.l + 1:  # adding val to end of list (potentially to empty list)
            return self.push_back(key=key, val=val)

        if i == 1:  # adding val to front of list (at least one val in list already exists)
            return self.push_front(key=key, val=val)
        ...
        insert_node = Node(key=key, val=val, nex=ith_node, prv=ith_node.prv)
        ith_node.prv = insert_node
        ith_node.prv.nex = insert_node
